// Modified from https://github.com/dreamyguy/validate-color
...
export const validateHTMLColorName = (color: string) => {
	let status = false;
	if (isString(color)) {
...
export const validateHTMLColorSpecialName = (color: string) => {
	let status = false;
	if (isString(color)) {
...
export const validateHTMLColorRgb = (color: string) => {
	if (isString(color)) {
		const regex =
...
export const validateHTMLColorHsl = (color: string) => {
	if (isString(color)) {
...
// 'deg': degrees | full circle = 360
export const validateHTMLColorHwb = (color: string) => {
	if (isString(color)) {
		const degRegex = `(-?([0-9]|[1-9][0-9]|1[0-9][0-9]|2[0-9][0-9]|3[0-5][0-9]|360)(deg)?)`;
...
export const validateHTMLColorLab = (color: string) => {
	if (isString(color)) {
...
export const validateHTMLColor = (color: string) => {
	if (
